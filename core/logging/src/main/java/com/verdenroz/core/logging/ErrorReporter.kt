...
/**
 * Logs [DataError] as non-fatal reports to [FirebaseCrashlytics]
 */
@Singleton
class ErrorReporter @Inject constructor(
    ...
    /**
     * Unified logging method for all [DataError] types
    ...
    fun logError(
        ...
        // Don't log errors in debug builds
        if (!BuildConfig.DEBUG) {
            ...
            if (error is DataError.Auth) {
                val isCriticalAuthError = when (error) {
